...
function initBarPage(data) {
  ...
  d3.select("input[name='range']")
    ...
    // записываем данные о текущих годах в label
  d3.select("label[name ='min']").text(range[0]);
  d3.select("label[name ='max']").text(range[1]);
  d3.select("label[name ='now']").text(selectedYear);
...
function renderBarPage() {
  ...
  // считаем scale, знаем что 118 элементов норм влезают и отступы ок
  var heightScale = displayedData.length / 118;
  
  // да бы размер svg элемента менялся каждый раз при пересчете
  // записываем тут максимальный height помноженный на scale
  d3.select("svg").attr("height", 1800 * heightScale + margin.top + margin.bottom );
  ...
  var labels = groups
    ...
    .attr("x", xScale(min))
    .attr("y", function(d) {
      return yScale(d.name) * 2 * heightScale; // чтоб отступы сверху были норм, тоже помнажаю на scale 
    })
    .attr("dy", "0.7em")
    .attr("dx", "-11em")
    .attr("class", "label");
  ...
  var bars = groups
    ...
    .attr("x", xScale(min))
    .attr("y", function(d) {
      return yScale(d.name) * 2 * heightScale;  // и тут
    })
    .attr("class", "bar");
}
...
function changeBarsYear() {
  ...
  // когда обновим дату, то и тут тоже надо перезаписать значение
  d3.select("label[name ='now']").text(selectedYear);
  renderBarPage();
}
